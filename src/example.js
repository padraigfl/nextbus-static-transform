...
results = rTags.reduce(function(acc, tag) {
  console.log(tag);
  var routeJson = readJSON(routeDataDir+'/'+tag+'.json', 'utf8');
  var scheduleJson = readJSON(scheduleDataDir+'/'+tag+'.json', 'utf8');//todo, export logic
  if(scheduleJson){
    return minifyRoutes.addScheduleData(tag, scheduleJson, acc.routes, acc.stops);
  }
  return acc;
}, results);
